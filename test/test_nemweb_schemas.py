...
def test_dispatch_region_sum_schema_fields():
    ...
    fields = DispatchRegionSumSchema.empty().columns
...
def test_dispatch_load_schema_fields():
    ...
    fields = DispatchLoadSchema.empty().columns
    assert "SETTLEMENTDATE" in fields
...
def test_bid_day_offer_schema_fields():
    ...
    fields = BidDayOfferDSchema.empty().columns
    assert "DUID" in fields
    assert "SETTLEMENTDATE" in fields
...
def test_generation_schemas_fields():
    ...
    genunits_fields = GENUNITSSchema.empty().columns
    dudetail_fields = DUDETAILSchema.empty().columns
...
def test_station_schemas_fields():
    ...
    station_fields = STATIONSchema.empty().columns
    stadualloc_fields = STADUALLOCSchema.empty().columns
...
def test_interconnector_schemas_fields():
    ...
    ic_fields = INTERCONNECTORSchema.empty().columns
    loss_fields = LOSSMODELSchema.empty().columns
...
def test_constraint_schemas_fields():
    ...
    gencon_fields = GENCONDATASchema.empty().columns
    spdregion_fields = SPDREGIONCONSTRAINTSchema.empty().columns
...
def test_schema_fields_match_table_columns():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        for ds_attr in dir(manager):
            ...
            schema_fields = set(schema_class.empty().columns)
...
def test_validate_against_schema_raises_on_error():
    ...
    df = pl.DataFrame({"SETTLEMENTDATE": [], "TOTALDEMAND": [], "archive_month": []}).cast(
        {
            "SETTLEMENTDATE": pl.Datetime,
            "TOTALDEMAND": pl.Float32,
        }
